...
void draw() {
  perspective(fov, float(width)/float(height), cameraZ/50000.0, cameraZ*50000.0);
  leveleo();
...
class Balls {
  ...
  void dibujar(int c1, int c2, int c3, int x0, int x1, int y0, int y1, int z0, int z1) {
  ...
  void vida(int x0, int x1, int y0, int y1, int z0, int z1, float t) {
...
void cabine() {
  ...
  cx = map(nmx, -500, 500, 0.8, -0.8);
  cy = map(nmy, -500, 500, -0.8, 0.8);
  rotateZ(cx);
  rotateX(cy);
